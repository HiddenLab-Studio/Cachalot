...
const FrenchExercise = () => {
    ...
    const [exercise, setExercise] = useState(data.currentExerciseId);
    ...
    useEffect( () => {
        ...
        frenchFunctions.init().then(() => {
            console.log("Init done (result: " + data.currentExerciseId + " )");
            setExercise(data.currentExerciseId);
            setIsLoading(false);
        ...
        return () => {
            data.currentExerciseType = undefined;
    ...
    async function onKeyPress(event) {
        if (event.key === "Enter") {
            let result = await frenchFunctions.getSolution();
    ...
    function handleState(type, newData){
        switch (type) {
            case "exercise":
                ...
                break;
            case "exerciseType":
                frenchFunctions.selectExerciseType(newData).then(() => {
